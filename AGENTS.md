...
- **USB mode + TinyUSB**: T-Dongle/Waveshare run `USBMode=hwcdc`; ESP32XInput's `tinyusb_init()` takes the USB port over from the built-in USB-Serial/JTAG — Serial console is lost at runtime on those boards, use UART0 (Serial0) for debug.
